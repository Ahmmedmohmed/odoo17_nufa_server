...
class AttendanceSheet(models.Model):
    ...
    def write(self, vals):
        res = super(AttendanceSheet, self).write(vals)
        if 'employee_id' in vals or 'date_from' in vals or 'date_to' in vals:
            self.onchange_employee()
        return res
    # fix deletion of contract on create also   
    @api.model_create_multi
    def create(self, vals):
        att_sheets = super().create(vals)
    ...
    def _cron_generate_attendance_sheet(self):
        ...
        for employee in employee_ids:
            try:
                attendance_sheet_id = att_sheet_obj.search(
                ...
                if not values.get('att_policy_id'):
                    _logger.warning(
                        (
                                'No Attendance Policy For Employee  %s ' % employee.name))
                    continue
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            for day in all_dates:
                ...
                work_intervals = calendar_id.att_get_work_intervals(att_sheet,
                                                                    day_start,
                                                                    day_end, tz)

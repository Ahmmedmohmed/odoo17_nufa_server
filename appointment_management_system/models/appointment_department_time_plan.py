...
class AppointmentDepartmentTimePlan(models.Model):
    _name = "appointment.department.time.plan"
    _description = 'Appointment Department Time Plan'
    ...
    def _check_hours_value(self):
        for record in self:
            if not 1 <= record.start_hour <= 24:
                raise ValidationError(_('Hours Value Must Be Between (1 - 24).'))

            if not 1 <= record.end_hour <= 24:
                raise ValidationError(_('Hours Value Must Be Between (1 - 24).'))
    ...
    @api.constrains('start_hour', 'start_minute', 'end_hour', 'end_minute', 'day', 'department_id')
    def _check_time_conflicts(self):
        for slot in self:
            ...
            same_day_slots = self.search([('id', '!=', slot.id), ('department_id', '=', slot.department_id.id), ('day', '=', slot.day)])

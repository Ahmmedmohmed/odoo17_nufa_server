...
class PosConfig(models.Model):
    ...
    wk_stock_type = fields.Selection([('available_qty', 'Available Quantity(On hand)'), ('forecasted_qty', 'Forecasted Quantity'), ('virtual_qty', 'Quantity on Hand - Outgoing Qty')], string='Stock Type', default='available_qty', required=True)
    ...
    def wk_pos_fetch_pos_stock(self, kwargs):
        ...
        pos_products_qtys = pos_products.with_context(location=location_id)._compute_quantities_dict(self._context.get('lot_id'), self._context.get('owner_id'), self._context.get('package_id'), self._context.get('from_date'), self._context.get('to_date'))
...
class ResConfigSettings(models.TransientModel):
    ...
    pos_wk_stock_type = fields.Selection(related='pos_config_id.wk_stock_type', readonly=False)
...
class PosSession(models.Model):
    ...
    def _loader_params_product_product(self):
        ...
        result['search_params']['fields'].extend(['qty_available', 'virtual_available', 'outgoing_qty', 'type'])
        return result
    ...
    def _get_pos_ui_product_product(self, params):
        ...
        location_id = picking_type.default_location_src_id.id
        wk_stock_type = self.config_id.wk_stock_type
    ...
    def get_pos_ui_product_product_by_params(self, custom_search_params):
        products = super().get_pos_ui_product_product_by_params(custom_search_params)
        picking_type = self.config_id.picking_type_id
        location_id = picking_type.default_location_src_id.id
        wk_stock_type = self.config_id.wk_stock_type
        wk_product_qtys = self.get_updated_qty_real_time(); 

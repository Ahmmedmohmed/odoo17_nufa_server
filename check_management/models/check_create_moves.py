...
class AccountMoveLine(models.Model):
    ...
    date_maturity = fields.Date(string='Due date', index=True, required=False,help="This field is used for payable and receivable journal entries. You can put the limit date for the payment of this line.")
...
class CreateMoves(models.Model):
    _name = "create.moves"
    _description = "Create Moves"

    def create_move_lines(self, **kwargs):
        required_keys = ['amount', 'src_currency', 'move', 'debit_account', 'credit_account', 'move_line']
        for key in required_keys:
            if key not in kwargs:
                raise KeyError(f"Missing required key in kwargs: {key}")

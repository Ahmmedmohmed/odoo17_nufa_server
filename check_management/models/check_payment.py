...
class NormalPayments(models.Model):
    ...
    name = fields.Char(string="", required=False, compute="get_title", readonly=True,store=True)
    ...
    payment_method = fields.Many2one(comodel_name="account.journal", string="Payment Journal", required=True,domain=[('type', 'in', ('bank', 'cash'))])
    ...
    state = fields.Selection(selection=[('draft', 'Draft'), ('posted', 'Posted'), ], default='draft',track_visibility='onchange')
    ...
    def action_confirm(self):
        for check in self.pay_check_ids:
            pay_amt = check.amount
            move = {
                'name': self.name + check.check_number,
            ...
            if self.send_rec_money == 'send':
                debit_account = [{'account': self.account_id.id, 'percentage': 100,
                                  'analytic_distribution': {self.analyitc_id.id: 100}}]
            ...
            self.env['create.moves'].create_move_lines(
                ...
                src_currency=self.account_id.currency_id,
                amount=pay_amt
            )
            check_line_val = {
                'check_number': check.check_number,
                'check_date': check.check_date,
                'check_bank': check.bank.id,
                'dep_bank': check.dep_bank.id,
                'state': 'holding' if self.send_rec_money == 'rece' else 'handed',
                'check_type': 'rece' if self.send_rec_money == 'rece' else 'pay',
                'amount': check.amount,
                'open_amount': check.amount,
                'type': 'regular',
                'notespayable_id': self.payment_method.default_debit_account_id.id,
                'notes_rece_id': self.payment_method.default_debit_account_id.id,

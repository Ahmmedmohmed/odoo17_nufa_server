...
class sale_day_book_wizard(models.TransientModel):
    ...
    def _compute_quantities_product_quant_dic(self,lot_id, owner_id, package_id,from_date,to_date,product_obj,data):
        ...
        if data['warehouse'] and not data['location_id']:
            ...
            for i in ware_check_domain:
                ...
                for i in loc_ids.view_location_id.child_internal_location_ids :
        ...
        domain_quant_end = [('product_id', 'in', product_obj.ids)] + domain_quant_loc + custom_domain
        ...
        if lot_id is not None:
            ...
            domain_quant_end += [('lot_id', '=', lot_id)]
        if owner_id is not None:
            domain_quant += [('owner_id', '=', owner_id)]
            domain_quant_end += [('owner_id', '=', owner_id)]
            domain_move_in += [('restrict_partner_id', '=', owner_id)]
            domain_move_out += [('restrict_partner_id', '=', owner_id)]
        if package_id is not None:
            domain_quant += [('package_id', '=', package_id)]
            domain_quant_end += [('package_id', '=', package_id)]
        ...
        quants_res_end = dict((item['product_id'][0], item['quantity']) for item in Quant.read_group(domain_quant_end, ['product_id', 'quantity'], ['product_id']))
        ...
        for product in product_obj.with_context(prefetch_fields=False):
            ...
            qty_available_end = quants_res_end.get(product_id, 0.0)
            res[product_id]['qty_available'] = float_round(qty_available, precision_rounding=rounding)
            res[product_id]['qty_available_end'] = float_round(qty_available_end, precision_rounding=rounding)
                ...
                for move in stock_move_line :
                    if move.picking_id.picking_type_id.code == "outgoing" :
                        if data['location_id'] :
                            ...
                            for i in data['location_id'].child_internal_location_ids :
    ...
    def print_exl_report(self):
        if xlwt:
            ...
            export_id = self.env['sale.day.book.report.excel'].create({'excel_file': base64.b64encode(fp.getvalue()), 'file_name': filename})
